...
export class BeatFractalEngine {
  ...
  // property access below isn't treated as an index-signature lookup under
  // noUncheckedIndexedAccess — every uniform is always populated in initGL.
  ...
  async connectAudio(audioEl: HTMLAudioElement, opts: { threshold?: number, cooldownMs?: number } = {}) {
    // Safari still ships AudioContext under a vendor-prefixed name.
  ...
  private pollAudio(dtMs: number) {
    ...
    let sum = 0
    for (let i = 0; i < 12; i++) sum += this.audioData[i] ?? 0
    const avg = sum / 12
  ...
  private randomPoint() {
    ...
    return pts[Math.floor(Math.random() * pts.length)] ?? this.center
  }
