...
**Status:** in progress (03-00 through 03-02 done, pending review).
...
| 03 | WS ticket auth: `POST /api/rt/ticket` → ~60s signed JWT → `partysocket` `query` → Worker verifies, strips, injects `x-user-*` headers → DO. | next |
