...
**Status:** in progress (03-00 and 03-01 done, pending review).
...
| 02 | Guest identity minting on client boot (`getSession()` → `signIn.anonymous()`), `useIdentity()`; never blocks render. | next |
