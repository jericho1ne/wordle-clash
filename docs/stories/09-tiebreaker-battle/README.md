...
room spectates. A beat-reactive WebGL fractal promoted from prototype to app
code is the arena background, pulsing on every server-confirmed
hit.
...
1. Epic 07's persisted `tiebreak` phase and `tiebreakPlayerIds` on
   `MatchSnapshot`.
2. Epic 08's `Beatmap` schema and checked-in beatmap data.

## Dance-off rules

Replaces the earlier "forthcoming" rules note:
...
|---|---|---|
| [00](./00-tiebreaker-battle-plan.md) | **Done:** stack plan: scoring numbers, sudden-death rule, route shape, rules update | Reviewable implementation contract |
| 01 | **Done:** promote the fractal prototype into app infrastructure and components; `/tiebreaker` playground with mock 2-player state, keyboard input, real beatmap | Verified in-browser — playable, fractals render, keyboard input scores |
...
- A full two-browser live walkthrough (two real player identities: tie a
  ...
  allowlist is hardcoded to `http://localhost:5173`, which was already occupied
  by another session. To run it: `pnpm dev`, set
  `GAMEPLAY_TEST_ANSWER=CLASH` in the server's local environment for a deterministic win, open two browser profiles
...
- Reconnect-mid-battle and the sudden-death (exact-tie) path are implemented
  in `#endDanceOff` but not exercised by a live test — worth
