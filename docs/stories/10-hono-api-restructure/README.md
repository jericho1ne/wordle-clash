...
The Worker entrypoint routes every request through a hand-rolled `if
(url.pathname === ...)` chain. `hono` (`^4.13.5`) has been a server dependency
since epic 00 for exactly this purpose —
...
02," that never got actioned. It has remained unused ever since.
...
None — this only touches the Worker entrypoint. Existing route handlers already
...
| [03](./03-route-tests.md) | Regression tests for every route via `@cloudflare/vitest-pool-workers` | `pnpm --filter @wordle-clash/server test` passes |
