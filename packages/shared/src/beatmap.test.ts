...
describe('beatmap schema', () => {
  ...
  it('rejects an unknown lane', () => {
    expect(() => parseBeatmap(beatmap([{ timeMs: 0, lane: 'jump' as never }]))).toThrow()
  })

  it('rejects a negative timeMs', () => {
    expect(() => parseBeatmap(beatmap([{ timeMs: -1, lane: 'up' }]))).toThrow()
  })
})
...
describe('usesAllLanes', () => {
  ...
  it('is true when all four lanes appear', () => {
    expect(usesAllLanes([
      { timeMs: 0, lane: 'up' },
      { timeMs: 200, lane: 'down' },
      { timeMs: 400, lane: 'left' },
      { timeMs: 600, lane: 'right' },
    ])).toBe(true)
  })
})
...
describe('findNearestEntryInLane', () => {
  const entries = [
    ...
    { timeMs: 3000, lane: 'up' as const },
  ]

  it('returns the closest entry in the requested lane within the window', () => {
    expect(findNearestEntryInLane(entries, 'up', 1050, 100)).toEqual({ timeMs: 1000, lane: 'up' })
  })

  it('returns null when the closest entry is outside the window', () => {
    expect(findNearestEntryInLane(entries, 'up', 2000, 100)).toBeNull()
  })
